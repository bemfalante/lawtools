...
def gerar_arquivos():
    for i in range(meses):
        ...
        mes_idx = (data_inicio.month - 1 + i) % 12 + 1
        ano_idx = data_inicio.year + (data_inicio.month - 1 + i) // 12

        data_atual = datetime(ano_idx, mes_idx, 1)
        # Obtém o último dia do mês para garantir datas válidas
        ultimo_dia = calendar.monthrange(ano_idx, mes_idx)[1]
        ...
        conteudo_sped_fiscal = f"""|0000|015|0|01{competencia}|{ultimo_dia}{competencia}|SUPERMERCADO PRECO JUSTO|00000000000100||RS|0000000|00000||A|1|
...
|9999|4|"""
        ...
        conteudo_sped_contrib = f"""|0000|005|0|01{competencia}|{ultimo_dia}{competencia}|SUPERMERCADO PRECO JUSTO|00000000000100|RS|0000000|00000|00|
...
OBS: Receitas monofásicas (NCM 2203, 2202) não segregadas."""
    ...
    print(f"Sucesso! {meses * 3} arquivos gerados na pasta {pasta_destino}.")

def compactar_arquivos():
    nome_zip = "recup_tributaria"
    shutil.make_archive(nome_zip, 'zip', pasta_destino)
    print(f"Sucesso! Pasta {pasta_destino} compactada como {nome_zip}.zip.")
